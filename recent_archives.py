...
def archive_at_season(date, versions):
    archive = []
    for entry in versions:
        sd = SortedCollection(versions[entry], key=archive_sort_function)
        try:
            previous = sd.find_le(archive_sort_function(date))
            archive.append(previous + '-' + entry + '.txt')
        except:
            pass
